...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default=".",
        help="Output folder for audio, captions, clips, and manifest (default: current directory).",
    )
    parser.add_argument(
        "--language",
        default="af",
...
def write_metadata(path: Path, info: dict[str, Any], caption_lang: str, caption_file: Path, audio_file: Path) -> None:
    root = path.parent.parent  # workspace root (metadata/ is one level down)
    def _rel(p: Path) -> str:
        try:
    ...
    payload = {
        ...
        "caption_file": _rel(caption_file),
        "audio_file": _rel(audio_file),
    }
...
def build_manifest_rows(
    ...
    for idx, segment in enumerate(segments, start=1):
        ...
        if root is not None:
            try:
                audio_path_str = str(clip_path.resolve().relative_to(root.resolve()))
            except ValueError:
                audio_path_str = str(clip_path.resolve())
        else:
            audio_path_str = str(clip_path.resolve())
        rows.append(
            {
